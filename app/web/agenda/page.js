...
export default function AgendaPage() {
  ...
  const [isLoading, setIsLoading] = useState(true); // ✅ Unifié ici
  ...
  // --- TRI ET REGROUPEMENT PAR MOIS ---
  // 1. On trie les évènements par date (du plus proche au plus lointain)
  const sortedEvents = [...filteredEvents].sort((a, b) => new Date(a.date_debut) - new Date(b.date_debut));

  // 2. On les regroupe dans un objet par mois (ex: { "Novembre 2026": [...], "Décembre 2026": [...] })
  const groupedEvents = sortedEvents.reduce((acc, event) => {
    const date = new Date(event.date_debut);
    // Formatage : ex: "novembre 2026"
    const monthYear = date.toLocaleString('fr-FR', { month: 'long', year: 'numeric' });
    // Majuscule à la première lettre : "Novembre 2026"
  ...
  // Fonction pour formater la date joliment pour la carte
      ...
      <section className="py-12 bg-gray-50 min-h-[60vh]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {isLoading ? (
            ...
            // ✅ NOUVEAU : Boucle sur les mois
            <div>
              {Object.entries(groupedEvents).map(([month, monthEvents]) => (
                <div key={month} className="mb-16 last:mb-0">
                  
                  {/* Titre du mois avec séparateur esthétique */}
                  ...
                  {/* Grille des évènements pour ce mois */}
                      ...
                      return (
                        <div key={event.id} className="group flex flex-col bg-white rounded-2xl overflow-hidden border border-gray-100 shadow-sm hover:shadow-xl hover:shadow-emerald-900/10 transition-all duration-300 hover:-translate-y-1">
                          ...
                          <div className="relative h-56 overflow-hidden">
                            <img
                              src={event.image || "/images/default-salle.png"} 
                              alt={event.type}
                          ...
                          <div className="flex flex-1 p-6 relative">
                            ...
                            <div className="flex flex-col flex-1 justify-between">
                              <div>
                                <h3 className="text-xl uppercase font-bold text-gray-900 mb-2 group-hover:text-emerald-700 transition-colors line-clamp-2">
                                  {event.nom_evenement || event.description}
                                </h3>
